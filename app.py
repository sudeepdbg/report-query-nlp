...
def stat_tiles(items: list):
    for col, (val, lbl, color) in zip(st.columns(len(items)), items):
        col.markdown(
            f'<div class="stat-tile" style="border-left:4px solid {color}">'
...
with st.sidebar:
    ...
    <div style="text-align:center;padding:20px 0">
        ...
        <div style="font-size:.7rem;color:#64748b;margin-top:4px">Rights Explorer · v5.0</div>
    </div>
    """, unsafe_allow_html=True)
...
def page_rights():
    ...
    st.markdown(f'<div class="page-sub">Content rights licensed-in for {reg} — windows, territories, exclusivity & expiry</div>', unsafe_allow_html=True)
...
def page_work_orders():
    ...
    if not df.empty:
        st.dataframe(df, use_container_width=True, hide_index=True, column_config={"quality_score": st.column_config.ProgressColumn("Quality", min_value=0, max_value=100, format="%.1f"), "cost": st.column_config.NumberColumn("Cost", format="$%,.0f")})
...
def page_chat():
    ...
    st.markdown(f'<div class="page-sub">Natural language rights interrogation · {reg} · 3-stage pipeline with interactive SQL chips</div>', unsafe_allow_html=True)
...
# ═══════════════════════════════════════════════════════════════════════════════
def db_save_pin(df: pd.DataFrame, fig, query_text: str, chart_type: str, key_prefix: str):
...
def db_suggested_queries(key_prefix: str = "sug"):
    ...
    st.markdown('<div style="background:#faf5ff;border:1px solid #ddd6fe;border-radius:12px;padding:16px 18px;margin-top:12px"><div style="font-size:.78rem;font-weight:700;color:#5b21b6;text-transform:uppercase;letter-spacing:.06em;margin-bottom:12px">💡 Suggested queries</div></div>', unsafe_allow_html=True)
    cols = st.columns(4)
...
def render_dynamic_dashboard(df: pd.DataFrame, chart_type: str, query_text: str, *, key_prefix: str="dyn", show_save_button: bool=True) -> Optional[go.Figure]:
    """Route df+chart_type to the best Plotly visualisation."""
    ...
    if len(df)==1 and len(df.columns) <= 2:
        ...
        if show_save_button: db_save_pin(df, None, query_text, chart_type, key_prefix)
        return None

    # 2. Table — always raw table, never auto-chart
    if chart_type == "table":
        if num_cols:
            mc = st.columns(min(4, len(num_cols)))
    ...
    if chart_type=="bar" and str_cols and num_cols:
        ...
        if color_col:
            fig = px.bar(top, x=yc, y=xc, color=color_col, orientation="h", title=title_txt, barmode="group", color_discrete_sequence=PT["colorway"], text_auto=True)
        else:
            n = len(top)
...
def _dash_tabbed(fig, df, num_cols, key_prefix, suffix, query_text, show_save, overflow=False):
    ...
    if show_save: db_save_pin(df, fig, query_text, "bar", key_prefix)
...
def page_custom_dashboard():
    ...
    st.markdown(f'<div class="page-sub">Natural language → dynamic visualisation · {reg} · 📌 {pin_count} pinned</div>', unsafe_allow_html=True)
    ...
    if run_clicked and query_text.strip():
        ...
        if error:
            ...
            st.warning("Could not generate SQL. "); db_suggested_queries("fail")
        else:
            res_df, db_err = execute_sql(sql, DB_CONN)
            if db_err:
                ...
                db_suggested_queries("no_results")
            else:
