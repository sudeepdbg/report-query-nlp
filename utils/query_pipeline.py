"""
query_pipeline.py  —  Foundry Vantage NL→SQL Pipeline  v5.3
=============================================================
Stage 1 : LLM (Ollama) with rule-based fallback  → QueryIntent
Stage 2 : Deterministic rule-based               → SQL
Stage 3 : Safety validation                      → validated SQL

FIXES v5.3 (all tested against live SQLite DB):
  1. _status_sql / _build_where double-AND bug fixed — _status_sql no longer
     prefixes "AND"; _build_where assembles conditions cleanly.
  2. DEAL_KW handler: vendor deals query runs against `deals` table only —
     no join to media_rights (deals has NO title_id column).
  3. deals_rights cross-intent: always uses content_deal→media_rights join,
     never the vendor `deals` table.
  4. Ollama prompt compacted to ~120 tokens so llama3.2:1b can handle it.
     Timeout raised to 30s. LLM_LAST_STATUS exposed for sidebar debug widget.
  5. parse_with_llm validates every LLM field before applying.
  6. match_method never overwritten after successful LLM call.
  7. _rw() f-string fixed (no nested quotes that break older Python).
  8. exhibition / restriction queries no longer swallowed by DNA_KW check.
"""
...
# ── Keyword sets ───────────────────────────────────────────────────────────────
DNA_KW       = {"do not air","do-not-air","dna","banned","blocked","not allowed"}
ELEMENTAL_KW = {"elemental","element","promo","trailer","featurette","asset","raw"}
...
ALL_MEDIA = [
    "PayTV","STB-VOD","SVOD","FAST","CatchUp",
    "StartOver","Simulcast","TempDownload","DownloadToOwn",
]
ONTOLOGY = {
...
def _apply_ontology(q: str) -> str:
    for phrase in sorted(ONTOLOGY, key=len, reverse=True):
        if phrase in q:
...
def _detect_cross_intent(i: QueryIntent) -> Optional[str]:
...
def _build_chips(intent: QueryIntent) -> list[dict]:
    chips = []
    dl = {
        "rights":"Rights","deals":"Deals","movies":"Movies","sales":"Sales Deals",
    ...
    chips.append({"id":"domain","kind":"domain","label":"Domain",
        "value": dl.get(intent.domain, intent.domain.replace("_"," ").title()),
    ...
    if intent.cross_intent:
        ...
        chips.append({"id":"cross_intent","kind":"cross_intent","label":"Join",
            "value": cl.get(intent.cross_intent, intent.cross_intent),
            "editable":False,"removable":False})
    for r in intent.regions:
        chips.append({"id":f"region_{r}","kind":"region","label":"Region",
            "value":r,"editable":False,"removable":True})
    for p in intent.platforms:
        chips.append({"id":f"platform_{p}","kind":"platform","label":"Platform",
            "value":p,"editable":False,"removable":True})
    if intent.title_hint:
        chips.append({"id":"title","kind":"title","label":"Title",
            "value":intent.title_hint,"editable":True,"removable":True})
    if intent.date_filter:
        chips.append({"id":"date","kind":"date","label":"Date",
            "value":intent.date_filter.label,"editable":True,"removable":True})
    if intent.expiry_days:
        chips.append({"id":"expiry_days","kind":"expiry_days","label":"Expiry Window",
            "value":f"{intent.expiry_days} days","editable":True,"removable":True})
    if intent.status_filter:
        chips.append({"id":"status","kind":"status","label":"Status",
            "value":intent.status_filter,"editable":True,"removable":True})
    if intent.movie_category:
        chips.append({"id":"movie_category","kind":"movie_category","label":"Category",
            "value":intent.movie_category,"editable":True,"removable":True})
    return chips

# ── Stage 1a: Rule-based preprocess ───────────────────────────────────────────
def preprocess(question: str, selected_region: str) -> QueryIntent:
    ...
    has_sales       = (any(kw in q for kw in SALES_KW)
                       or any(x in q for x in ["netflix","amazon","buyer"]))
    ...
    intent = QueryIntent(
        raw_question=question, normalised=q_norm,
        regions=regions, platforms=platforms,
        ...
        has_title=has_title, has_deal_word=has_deal_word,
...
# ── Stage 1b: LLM (Ollama) ────────────────────────────────────────────────────
_VALID_DOMAINS    = {"rights","deals","movies","sales","do_not_air",
                     "expiry","work_orders","titles","elemental"}
_VALID_CROSS      = {"deals_rights","title_health","expiry_sales",
                     "workorder_rights","movie_dna","movie_sales","title_sales"}
_VALID_STATUSES   = {"Active","Expired","Pending"}
_VALID_CATEGORIES = {"Theatrical","Library","HBO Original","Direct-to-Streaming"}


def call_ollama(prompt: str) -> Optional[str]:
    """POST to Ollama. Updates LLM_LAST_STATUS for sidebar widget."""
    try:
        resp = requests.post(
            OLLAMA_URL,
        ...
        if resp.status_code == 200:
            ...
            return text
        LLM_LAST_STATUS.update({"success": False,
...
def parse_with_llm(question: str, selected_region: str) -> Optional[QueryIntent]:
    """
    Compact prompt (≈120 tokens) so llama3.2:1b can handle it reliably.
    Every field is validated before overriding the rule-based base.
    Returns None on any failure → caller falls back to preprocess().
    """
    prompt = (
        'Extract media-rights query intent. Reply ONLY with JSON, no prose.\n'
        f'Question: "{question}"  Default region: "{selected_region}"\n'
        'Required JSON (use null for unknown):\n'
        '{"domain":"rights|deals|movies|sales|do_not_air|expiry|work_orders|titles|elemental",'
        '"cross_intent":"deals_rights|title_health|expiry_sales|workorder_rights|'
        'movie_dna|movie_sales|title_sales|null",'
    ...
    if not jm:
        LLM_LAST_STATUS.update({"success": False,
                                 "error": f"No JSON in response: {resp[:80]}"})
        logger.warning("Ollama: no JSON found in: %s", resp[:200])
        return None
    ...
    try:
        ...
        logger.warning("Ollama JSON error: %s — raw: %s", e, resp[:200])
        return None

    # Start from rule-based, override only valid LLM fields
    ...
    if isinstance(data.get("platforms"), list) and data["platforms"]:
    ...
    logger.info("LLM parsed: domain=%s regions=%s cross=%s",
                base.domain, base.regions, base.cross_intent)
    return base
...
def _rw(regions: list[str], col: str = "region") -> str:
    if not regions:
        return "1=1"
    if len(regions) == 1:
        return "UPPER(" + col + ") = '" + regions[0] + "'"
    vals = "','".join(regions)
    return "UPPER(" + col + ") IN ('" + vals + "')"

def _plat(platforms: list[str], col: str) -> str:
    if not platforms:
        return "1=1"
    return "(" + " OR ".join(col + " LIKE '%" + p + "%'" for p in platforms) + ")"

def _title_like(hint: str, col: str = "title_name") -> str:
    safe = hint.replace("'", "''").replace(";", "")[:100]
    return "LOWER(" + col + ") LIKE '%" + safe.lower() + "%'"
...
def _status_cond(status: Optional[str], col: str = "status") -> str:
    """
    FIX v5.3: returns a bare WHERE condition (no leading AND).
    Caller must join via _build_where which adds AND between parts.
    """
    if not status:
        return ""
    if status == "Pending":
        return f"({col}='Pending' OR {col}='Under Negotiation')"
    return f"{col}='{status}'"

def _build_where(*parts: str) -> str:
    """
    FIX v5.3: strips any leading 'AND ' from each part before joining,
    preventing double-AND when callers pass pre-prefixed strings.
    """
    cleaned = []
    for p in parts:
        p = p.strip()
        if not p or p.upper() in ("1=1", "AND 1=1"):
            continue
        # Strip accidental leading AND
...
def generate(intent: QueryIntent) -> tuple[str, Optional[str], str]:  # noqa: C901
    q  = intent.normalised
    r  = intent.regions
    p  = intent.platforms
    ...
    rw_t   = _rw(r, "t.region")
    rw_cd  = _rw(r, "cd.region")    # content_deal alias
    rw_sd  = _rw(r, "sd.region")
    rw_dna = _rw(r, "dna.region")
    rw_wo  = _rw(r, "wo.region")
    plat_f = ("AND " + _plat(p, "mr.media_platform_primary")) if p else ""
    dsql   = _date_sql(intent.date_filter)
    ...
    # ── Cross-intent: deals + rights ─────────────────────────────────────────
    # ALWAYS uses content_deal → media_rights. Never the vendor `deals` table
    # (which has NO title_id column).
    if ci == "deals_rights":
        if any(kw in q for kw in
               ["breakdown","summary","count","how many","by region","compare","overview"]):
            sql = f"""
                SELECT cd.region,
    ...
    if ci == "expiry_sales":
        sql = f"""
            ...
            LEFT JOIN sales_deal sd
                ON mr.title_id = sd.title_id AND {rw_sd} AND sd.status='Active'
    ...
    if ci == "workorder_rights":
        sql = f"""
            ...
            LEFT JOIN media_rights mr
                ON t.title_id = mr.title_id AND {rw_mr} AND mr.status='Active'
            WHERE {rw_wo}
    ...
    if ci == "movie_dna":
        sql = f"""
            SELECT m.movie_title, m.content_category, m.genre,
                   ...
                   CASE WHEN COUNT(dna.dna_id) > 0
                        THEN '🚫 Flagged' ELSE '✅ Clean' END AS dna_status
            FROM movie m
    ...
    if ci == "movie_sales":
        sql = f"""
            ...
            ORDER BY sd.deal_value DESC NULLS LAST,
                     m.box_office_gross_usd_m DESC
            LIMIT 150
        """
        return sql.strip(), None, "table"
    ...
    # NOTE: exhibition / restriction checked BEFORE DNA_KW so 'restrict'
    # in a query like 'exhibition restrictions' doesn't misroute to do_not_air.
    # ─────────────────────────────────────────────────────────────────────────

    if "exhibition" in q and "restrict" in q:
        sql = f"""
            SELECT mr.title_name, mr.region,
    ...
    if intent.has_movie:
        ...
        if any(kw in q for kw in ["breakdown","by genre","by category","count","how many","genre"]):
            ...
            sql = f"""
                ...
                LEFT JOIN media_rights mr ON mr.title_id = t.title_id AND {rw_mr}
                WHERE 1=1 {cat_m}
                GROUP BY {gc} ORDER BY films DESC
            """
            return sql.strip(), None, "bar"
        # Default movie list
        sql = f"""
    ...
    if any(kw in q for kw in SALES_KW):
        ...
        sql = f"""
            SELECT sd.sales_deal_id, sd.deal_type, sd.title_name, sd.buyer,
                   sd.territory, sd.media_platform,
                   sd.term_from, sd.term_to,
                   sd.deal_value, sd.currency, sd.status, sd.region
            FROM sales_deal sd
            WHERE {rw} ORDER BY sd.deal_value DESC LIMIT 200
        """
        return sql.strip(), None, "table"
    ...
    # ── Vendor deals table ────────────────────────────────────────────────────
    # FIX v5.3: `deals` has NO title_id — never join to media_rights.
    # _status_cond() returns bare condition (no AND prefix) → _build_where joins cleanly.
    if any(kw in q for kw in DEAL_KW):
        rw_d    = _rw(r, "d.region")
        st_cond = _status_cond(intent.status_filter, "d.status")
        dt_cond = _date_sql(intent.date_filter).lstrip("AND ").strip()
        where   = _build_where(rw_d, st_cond, dt_cond)
        ...
        if "quality" in q or "vendor" in q:
            sql = f"""
                SELECT vendor_name, region,
                       COUNT(*) AS orders, AVG(quality_score) AS avg_quality,
    ...
    if intent.has_title:
        ...
        sql = f"""
            SELECT t.title_id, t.title_name, t.title_type, t.content_category,
                   t.genre, t.release_year, t.controlling_entity,
...
_DANGEROUS = re.compile(
    r'(;\s*drop|;\s*delete|;\s*update|;\s*insert|xp_|exec\s*\(|union\s+select)',
    re.IGNORECASE,
)
...
def validate(sql: str, intent: QueryIntent) -> tuple[str, Optional[str]]:
    ...
    if _DANGEROUS.search(sql):
        return sql, "Dangerous SQL pattern blocked"
    for r in intent.regions:
...
# ── Public API ─────────────────────────────────────────────────────────────────
def parse_query(
    question: str, selected_region: str = "NA"
) -> tuple[str, Optional[str], str, str, QueryIntent]:
    """
    Returns (sql, error, chart_type, region_ctx, intent).
    intent.match_method = 'llm' or 'rule' — never overwritten after parse.
    LLM_LAST_STATUS is updated on every attempt so sidebar widget is live.
    """
    try:
        intent = None
        if USE_LLM:
        ...
        if intent is None:
            # Ollama failed or disabled — fall back to rule-based
            intent = preprocess(question, selected_region)
            # match_method already 'rule' from preprocess()

        region_ctx = (
            " vs ".join(intent.regions) if len(intent.regions) > 1
            else intent.regions[0]
        )
        ...
        logger.error("parse_query exception: %s", e, exc_info=True)
        dummy = QueryIntent(
