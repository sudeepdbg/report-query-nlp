"""
database.py — Foundry Vantage Rights Explorer Database Module
═════════════════════════════════════════════════════════════
Handles SQLite initialization, schema creation, seeding, and query execution.
Compatible with app.py v5.0 and query_pipeline.py v5.3.
"""
...
GENRES         = ["Drama", "Thriller", "Fantasy", "Sci-Fi", "Comedy", "Action", "Historical", "Crime", "Documentary", "Horror", "Animation", "Reality"]
LANGUAGES      = ["English", "Spanish", "French", "German", "Italian", "Japanese", "Korean", "Mandarin", "Hindi", "Portuguese", "Arabic", "Dutch"]
...
DEAL_TYPES       = ["Output Deal", "Library Buy", "First-Look Deal", "Co-Production", "Licensing Agreement", "Distribution Deal", "Volume Deal"]
...
def _seed_vendors(cur):
    ...
    logger.info(f"Vendors: {len(rows)} seeded")
...
def _seed_work_orders(cur, title_ids):
    ...
    logger.info(f"Work Orders: {len(rows)} seeded")
...
def _seed_sales(cur, title_ids):
    import random
    today  = datetime.now()
    buyers = ["Netflix", "Amazon", "Disney+", "Apple TV+", "Peacock", "Paramount+", "Hulu", "BritBox", "Stan", "Hotstar"]
    cur.execute("SELECT title_id, title_name FROM title")
    title_name_map = {row[0]: row[1] for row in cur.fetchall()}
    rows = []
...
def init_database(db_path="foundry.db"):
    """Initialize the SQLite database, create schema, and seed data."""
    import random
...
def log_query(conn, session_id: str, user_id: Optional[str], region: str,
              persona: str, question: str, generated_sql: str,
              intent_domain: str, cross_intent: bool, latency_ms: float,
              success: bool, error_message: str, rows_returned: int,
              chart_type: str) -> Optional[int]:
