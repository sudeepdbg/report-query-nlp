"""
query_parser.py — Foundry Vantage NL→SQL engine (FIXED v2.0)
✅ Multi-region support (LATAM and EMEA in single query)
✅ Date range parsing (last 60 days, last 3 months, etc.)
✅ Cross-table joins (Deals + Rights together)
...
def _region_where(regions, col="region"):
    """FIXED: Handle multiple regions properly"""
    if not regions:
        return "1=1"
    if len(regions) == 1:
...
def _detect_cross_intent(q):
    """
    Detects when user wants data from 2+ tables
...
class QueryParser:
    ...
    def generate_sql(cls, question, selected_region):
        ...
        # If regions found in query text, USE THEM (override sidebar)
        if text_regions:
        ...
        rw_deals = _region_where(regions, "region")  # For deals table
        ...
        if any(kw in q for kw in DEAL_KW) and not any(kw in q for kw in RIGHTS_KW):
            ...
            where_conditions = [rw_deals]
            if stat_f:
                where_conditions.append(stat_f.strip())
            if date_filter and date_filter != "1=1":
