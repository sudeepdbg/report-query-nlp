...
def _extract_regions(q):
    """FIXED: Correctly extracts ALL regions mentioned in query"""
    found_regions = []
    q_upper = q.upper()
    for r in REGION_CANONICAL:
        # Check if region code exists in query (word boundary check not strictly needed for codes like NA/EMEA)
        if r in q_upper:
...
class QueryParser:
    ...
    def generate_sql(cls, question, selected_region):
        ...
        # ── FIXED: Region Resolution Logic ───────────────────────────────────
        # 1. Extract regions mentioned in the query text
        text_regions = _extract_regions(q)
        
        # 2. If regions found in text, USE THEM (override sidebar default)
        #    This fixes "LATAM and EMEA" being ignored
        if text_regions:
        ...
        if cross == "title_health":
            ...
            sql = f"""
                ...
                FROM title t
                LEFT JOIN media_rights mr ON t.title_id = mr.title_id AND UPPER(mr.region) = '{regions[0]}'
                LEFT JOIN do_not_air dna ON t.title_id = dna.title_id AND dna.active = 1
                LEFT JOIN sales_deal sd ON t.title_id = sd.title_id AND UPPER(sd.region) = '{regions[0]}'
        ...
        if cross == "expiry_sales":
            ...
            sql = f"""
                SELECT mr.title_name, mr.media_platform_primary AS rights_platform, mr.territories,
                       ...
                       mr.exclusivity, mr.rights_type,
                       sd.buyer AS sold_to, sd.deal_value AS sales_value, sd.media_platform AS sales_platform,
                ...
                LEFT JOIN sales_deal sd ON mr.title_id = sd.title_id AND UPPER(sd.region) = '{regions[0]}'
                WHERE UPPER(mr.region) = '{regions[0]}' AND mr.status = 'Active'
        ...
        if cross == "workorder_rights":
            sql = f"""
                ...
                LEFT JOIN media_rights mr ON t.title_id = mr.title_id AND UPPER(mr.region) = '{regions[0]}' AND mr.status = 'Active'
        ...
        if cross == "movie_dna":
            ...
            sql = f"""
                ...
                LEFT JOIN media_rights mr ON mr.title_id = t.title_id AND UPPER(mr.region) = '{regions[0]}' AND mr.status = 'Active'
        ...
        if cross == "movie_sales":
            ...
            sql = f"""
                ...
                LEFT JOIN sales_deal sd ON sd.title_id = t.title_id AND UPPER(sd.region) = '{regions[0]}'
                LEFT JOIN media_rights mr ON mr.title_id = t.title_id AND UPPER(mr.region) = '{regions[0]}' AND mr.status = 'Active'
                WHERE 1=1 {cat_f}
        ...
        if cross == "title_sales":
            ...
            sql = f"""
                SELECT t.title_name, t.title_type, t.content_category,
                       mr.media_platform_primary AS rights_platform, mr.term_to AS rights_expiry,
                       CAST(JULIANDAY(mr.term_to)-JULIANDAY('now') AS INTEGER) AS rights_days_left,
                       mr.status AS rights_status,
                       sd.buyer, sd.media_platform AS sold_platform, sd.deal_value, sd.currency,
        ...
        if _is_movie_query(q):
            ...
            if "franchise" in q:
                sql = f"""
                    ...
                    FROM movie m
                    LEFT JOIN title t ON t.movie_id = m.movie_id
                    LEFT JOIN media_rights mr ON mr.title_id = t.title_id AND UPPER(mr.region) = '{regions[0]}'
            ...
            if any(kw in q for kw in EXPIRY_KW):
                ...
                sql = f"""
                    SELECT m.movie_title, m.content_category, m.genre,
                           m.box_office_gross_usd_m AS box_office_usd_m,
                           mr.media_platform_primary, mr.territories, mr.term_to,
                           CAST(JULIANDAY(mr.term_to)-JULIANDAY('now') AS INTEGER) AS days_remaining,
        ...
        if any(kw in q for kw in DEAL_KW) and not any(kw in q for kw in RIGHTS_KW):
            ...
            # Handle date ranges for deals (e.g. "in 30 days")
            date_filter = "1=1"
            if "30 days" in q: date_filter = "deal_date >= DATE('now', '-30 days')"
            elif "60 days" in q: date_filter = "deal_date >= DATE('now', '-60 days')"
            elif "last month" in q: date_filter = "deal_date >= DATE('now', '-1 month')"
            ...
            if any(kw in q for kw in ["vendor", "by vendor", "breakdown", "by type", "value"]):
                ...
                sql = f"""
                    ...
                    FROM deals WHERE {rw_deals} {stat_f} AND {date_filter}
        ...
        if any(kw in q for kw in TITLE_KW):
            title_f = f"WHERE {rw_t}" + (f" AND {_title_like(title_hint,'t.title_name')}" if title_hint else "")
