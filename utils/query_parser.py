...
class QueryParser:
    ...
    def generate_sql(cls, question, selected_region):
        ...
        # Region resolution - IMPROVED to handle multiple regions
        ...
        date_start, date_end, date_filter = _extract_date_range(q)
        ...
        # ── 11. Deals (vendor contracts) ──────────────────────────────────────
        # IMPROVED: Now handles date ranges and multiple regions
        if any(kw in q for kw in DEAL_KW) and not any(kw in q for kw in RIGHTS_KW):
            ...
            where_conditions = [f"UPPER(region) IN ('{','.join(regions)}')"]
            if stat_f:
                where_conditions.append(stat_f.strip())
            if date_filter and date_filter != "1=1":
                # Apply date filter to deal_date column
                where_conditions.append(date_filter.replace("date", "deal_date"))
